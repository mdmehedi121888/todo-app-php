...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ++$cnt;
        echo "<tr><td>" . $cnt . "</td><td id=" . $row['id'] . ">" . $row["title"] . "</td> <td>" . $row["date"] . " </td>
        <td ><button class='border-0'><a class='mx-3' onclick='markDone(" . $row['id'] . ")'><i class='fa-solid fa-check fs-4 cursor-pointer'></i></a></button>
